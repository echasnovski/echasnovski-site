...
::: {.callout-note}
Beware that, although unlikely, some information can become outdated. The Neovim's own documentation is always the higher authority than this post. From the running Neovim instance execute `:h vim.pack` to read the help and `:h vim.pack-examples` to read suggested workflows for common use cases.
:::
...
![](2026-03-10-a-guide-to-vim-pack/demo-vim-pack-add-confirm.png)
...
Adding plugin via `vim.pack.add()` makes it "active" or "loaded" in current session.
...
- It makes possible to comment out the (part of) `vim.pack.add()` call and some plugins will not load in the next session. Almost as if they don't exist at all.
...
- It is used as a "latest working state" reference when bootstrapping the config on the new machine. In this case during **the very first `vim.pack.add()` call** all plugins that are present in the lockfile but absent on disk are installed all at once. This ensures that even [lazy loaded](#lazy-loading) plugins are installed right away.
...
Executing `:lua vim.pack.update()` (without arguments) will update all plugins and `:lua vim.pack.update({ 'mini.nvim' })` will update selected installed plugins specified by their name.
...
- By default show a new tabpage with information about the update. This is known as a confirmation buffer. To skip the confirmation step, all pending updates can be applied immediately by setting `force = true` option of `vim.pack.update()`.
...
The goal of the confirmation buffer is to show update details for the user to read, confirm (execute `:write`) or deny (close the window, like with `:quit`) the update. Here is an example of how it looks:

![](2026-03-10-a-guide-to-vim-pack/demo-confirmation-buffer.png)

There are several buffer-local nice things to improve user experience:

- The `]]` and `[[` mappings navigate through plugin sections.

- There is a so-called "in-process" (i.e. created in Neovim session by Neovim code) LSP server enabled specifically in confirmation buffer. It provides several features in an LSP compatible way. This means that they will work both with [default LSP mappings](https://neovim.io/doc/user/helptag.html?tag=lsp-defaults) and with user's any custom mappings.

    The supported LSP methods are:

    - ['textDocument/documentSymbol'](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.buf.document_symbol()) to show structure of the buffer. By default opens a location list, but will work with, for example, any picker.

    - ['textDocument/hover'](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.buf.hover()) - show more information at cursor, like details of particular pending change or newer tag. By default opens a floating window showing the change; execute the hover action again to jump into the window or move cursor for the window to disappear.
...
- `:lua vim.pack.update(nil, { offline = true })` - show the current state of plugins without downloading new changes from the source. Useful to interactively explore installed plugins.
...
- Remove any loading logic of the plugin from the config. Otherwise it will be reinstalled in the next session.
...
- Use [`vim.pack.del()`](https://neovim.io/doc/user/helptag.html?tag=vim.pack.del()) to delete plugin(s) from disk. This will also remove the plugin from the [lockfile](#lockfile). Here is an example: `:lua vim.pack.del({ 'nvim-lspconfig', 'nvim-treesitter' })`.
    ...
    **Do not delete plugin directory by hand**. This will not adjust lockfile, which means the plugin will be reinstalled on the next startup.
...
It should report any known/common issues together with best-effort suggestion on how to fix it. For example it will:
...
There is a chance that you already use a plugin manager, in which case in order to use `vim.pack` you'd have to migrate from the previous one. And my personal suggestion would be to do it, as `vim.pack` being built-in means that it 1) has dedicated and knowledgeable people maintaining it; and 2) has basically the same stability guarantees as Neovim core itself. The downside might be a slower release cycle than a regular plugin, so you'd have to wait longer for possible fixes and new features.

Depending on the complexity and the number of used plugins, it might be a complex and tedious task. But in a nutshell:

1. Update the config to use `vim.pack` instead of the previous plugin manager. Make sure that there is no single use of the previous plugin manager whatsoever (otherwise it will become confusing to locate problems).

2. Make sure that plugins and accompanying files from the previous plugin manager are not on disk. This might be crucial to not have several plugins mixed.
...
- Adjust a temporary config (see [`:h $NVIM_APPNAME`](https://neovim.io/doc/user/helptag.html?tag=$NVIM_APPNAME)) at your own pace without the pressure of not having the fully working setup. You can start either with a full copy of the "main" config or start fresh.

    For example, create `~/.config/nvim-vim-pack` directory. Either empty to start fresh or by copying `~/.config/nvim`. Adjusting `nvim-vim-pack` config will have no impact on the "main" `nvim` config. Use `NVIM_APPNAME=nvim-vim-pack nvim` to start Neovim with the `nvim-vim-pack` config. When you feel like done, move the content back to `~/.config/nvim` (it will reinstall `vim.pack` plugins into a new ["data" standard path](https://neovim.io/doc/user/helptag.html?tag=standard-path)).
...
In many ways, `vim.pack` is better and slicker version of 'mini.deps' which is why it will eventually be recommended instead of 'mini.deps'. So the migration should be rather painless. But here are a couple of pointers:
...
- **Do not forget** to remove plugins (located in `site/pack/deps` subdirectory of ["data" standard path](https://neovim.io/doc/user/helptag.html?tag=standard-path)) and possible `mini-deps-snap` snapshot file. Otherwise there will be conflicts (since 'mini.deps' also uses [plugin package](#plugin-packages)).

You can see an example of a real config migration [in MiniMax](https://nvim-mini.org/MiniMax/configs/diffs/nvim-0.11_nvim-0.12/): the main theme of moving from `nvim-0.11` to `nvim-0.12` reference config is changing plugin manager from 'mini.deps' to `vim.pack`.
...
The [folke/lazy.nvim](https://lazy.folke.io/) is the most used plugin manager at the time of this writing. And rightly so: it is very capable with lots of features. Ironically, this itself makes it not very suitable to be part of Neovim as most of the features come with great code and maintenance complexity. Plus the whole idea of treating lazy loading as the main goal of a plugin manager does not sit well with Neovim core team.

Not all 'lazy.nvim' capabilities are one-to-one reproducible by design. My best suggestion would be to follow [both approaches suggested earlier](#migrate): use temporary config and start with [simple approach](#single-vim-pack-add) first.

Couple of pointers:

- Use full URL as source (as string or `src` field) and a single `version` instead of `branch` / `tag` / `commit` / `version` / `pin` fields.

- Instead of the `opts` field to configure plugin, use a dedicated way to configure a plugin (should be mentioned in plugin's documentation/README) after `vim.pack.add`ing it. Typically it is something like `require('plugin').setup(opts)`, which is done by 'lazy.nvim' behind the scenes, but not by `vim.pack.add()`.
...
- If you use [structured plugins](https://lazy.folke.io/usage/structuring), it should be okay to start with [many `vim.pack.add()`](#many-vim-pack-add) config organization. Instead of `return { 'user/repo', ... }` it has to be a call to `vim.pack.add({ 'https://github.com/user/repo' })` (either right away or registered for lazy loading; along with configuring the plugin).
...
Here is a migration example partially based on the [example from 'lazy.nvim' documentation](https://lazy.folke.io/spec/examples):

- 'lazy.nvim' "simple" config (excluding the part of bootstrapping 'lazy.nvim'):
      ...
      spec = {
        {
          ...
          lazy = false,
          priority = 1000,
          config = function() vim.cmd('colorscheme miniwinter') end,
        },
        {
          'nvim-mini/mini.ai',
          opts = {
